...
var visibleDinerCodes = new Array();
...
function filterClick( code ){
	if( visibleDinerCodes[code] == null ){
		visibleDinerCodes[code] = new Array();
	}
	if( document.getElementById( code + "_FILTER").checked ){
		for( var dinerCode in allDiners ){
			var filterFoods = document.getElementById( allDiners[dinerCode] + "_FOOD" ).childNodes;
			for( var filterFood in filterFoods )
				if( filterFoods[filterFood].id == code + "_KEYWORD"){
					visibleDinerCodes[code].push( allDiners[dinerCode] );
				}
		}
	}
	else{
	...
	for( var dinerCode in allDiners ){
		var isVisible = false;
		for( var visibleDinerCodeFilter in visibleDinerCodes){
			if( indexOf( visibleDinerCodes[visibleDinerCodeFilter], allDiners[dinerCode] ) != -1 ){
				isVisible = true;
				break;
			}
		...
		if( isVisible ){
			document.getElementById( allDiners[dinerCode] + "_ALL" ).style.display = "";
		} else{
			document.getElementById( allDiners[dinerCode] + "_ALL" ).style.display = "none";
		}
	}
}
